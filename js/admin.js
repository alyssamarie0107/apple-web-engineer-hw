    ...
    const displayMedLgRoomsPending = () => {
        db.collection("conference-rooms").get().then((querySnapshot) => {
            querySnapshot.forEach((doc) => {
                ...
                // display rooms that need approval in table
                // med & large rooms are the only rooms that would have status 'request pending' or 'request still pending'
                if (data.status === "request pending" || data.status === "request still pending") {
                    let row =
                        `
                        <tr>
                            <td>${data.name}</td>
    ...
    roomNameDropdown.addEventListener('change', (e) => {
        ...
        docRef.get().then((doc) => {
            if (doc.exists) {
                ...
                // if room size is medium and admin approves, change room status to unavailable from 'request pending'
                if (data.size === "medium") {
                    ...
                    approveConfirmBtn.addEventListener('click', (e) => {
                        ...
                        docRef.set({
                            ...
                            .then(() => {
                                ...
                                alert(`Conference Room: ${data.name} approved successfully by admin!`);
                                // reload page
                                location.reload();
                            })
                            .catch((error) => {
                    ...
                    approveConfirmBtn.addEventListener('click', (e) => {
                        docRef.set({
                            ...
                            .then(() => {
                                console.log("document successfully updated!");
                                alert(`Conference Room: ${data.name} approved successfully by admin! Still needs manager approval.`);
                                // reload page
                                location.reload();
                            })
                            .catch((error) => {
